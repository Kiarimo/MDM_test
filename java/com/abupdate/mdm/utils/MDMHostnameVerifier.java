 ...
 */
public final class MDMHostnameVerifier implements HostnameVerifier {

    public static final MDMHostnameVerifier INSTANCE = new MDMHostnameVerifier();
    private static final Pattern VERIFY_AS_IP_ADDRESS = Pattern.compile("([0-9a-fA-F]*:[0-9a-fA-F:.]*)|([\\d.]+)");
    private static final int ALT_DNS_NAME = 2;
    private static final int ALT_IPA_NAME = 7;
    private final String hostname = ServerApi.getHostName();//String.valueOf(new char[]{'l', 'o','c', 'k', '.', 'e', 'l', 'e', 'p', 'h', 'a','n', 't', 't', 'e', 'k', '.', 'c', 'o', 'm',}); //"lock.elephanttek.com";

    private MDMHostnameVerifier() {
    }
    ...
    private boolean verifyIpAddress(String ipAddress, X509Certificate certificate) {
        ...
        LogUtils.d(altNames.size() + "");
        for (int i = 0, size = altNames.size(); i < size; i++) {
            LogUtils.d(altNames.get(i));
    ...
    private boolean verifyHostName(String hostName, String pattern) {
        if (!hostname.endsWith(hostName)) {
            LogUtils.e("verifyHostName failed, " + hostName + " & " + hostname);
            return false;
        }
